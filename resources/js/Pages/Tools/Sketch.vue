<template>
 <div class="flex flex-col h-screen bg-cream font-body overflow-hidden">

 <!-- Header -->
 <header class="flex items-baseline gap-6 px-6 py-3 border-b-4 border-ink bg-cream shrink-0">
 <h1 class="font-display text-sm font-bold tracking-tight">
 sketch <span class="text-coral">→</span> digital
 </h1>
 <p class="text-xs text-warm-muted italic hidden sm:block">
 ...
 <!-- Sidebar -->
 <aside class="w-56 shrink-0 border-r-2 border-warm-border bg-warm-surface flex flex-col gap-5 p-4 overflow-y-auto">

 <!-- 01 Image -->
 <section>
 <h3 class="sidebar-heading">01 — Image</h3>
 <button
 class="w-full border-2 border-dashed border-warm-border hover:border-coral hover:bg-coral/10 text-warm-muted hover:text-coral text-xs py-4 px-2 text-center leading-relaxed transition-colors cursor-pointer"
 :class="{ 'border-coral bg-coral/10 text-coral': isDragging }"
 ...
 <span>Threshold</span>
 <span class="font-display text-coral font-bold">{{ threshold }}</span>
 </div>
 <input
 ...
 :class="invert ? 'bg-coral' : 'bg-warm-muted'"
 ...
 <p class="text-xs text-warm-muted italic mt-1">Use invert for white ink on dark paper</p>
 </section>
 ...
 class="font-display w-full text-left px-3 py-2 text-xs font-bold tracking-wide border-2 border-ink bg-cream hover:bg-ink dark:hover:bg-[#1C1C1E] hover:text-white transition-colors disabled:opacity-30 disabled:cursor-not-allowed mb-2"
 @click="enterCropMode"
 >
 ⌗ Set Crop Region
 </button>
 ...
 <div class="text-[10px] leading-relaxed p-2 border border-coral text-coral bg-coral/10 italic mb-2">
 Drag on canvas to select crop region.
 </div>
 <div class="flex gap-1">
 <button
 class="font-display flex-1 px-2 py-1.5 text-xs font-bold border-2 border-mint bg-mint text-ink hover:bg-mint/80 transition-colors"
 @click="confirmCrop"
 >✓ Confirm</button>
 <button
 class="font-display flex-1 px-2 py-1.5 text-xs font-bold border-2 border-warm-border hover:border-ink transition-colors"
 @click="cancelCrop"
 >✕ Cancel</button>
 </div>
 </template>
 ...
 <div class="text-[10px] p-2 border border-warm-border text-warm-muted mb-2 leading-relaxed">
 Crop active:
 <span class="font-display text-coral font-bold">
 {{ cropRect.w }}×{{ cropRect.h }}
 </span>
 px
 </div>
 <button
 class="font-display w-full text-left px-3 py-1.5 text-xs border-2 border-warm-border hover:border-ink transition-colors"
 @click="clearCrop"
 >✕ Clear Crop</button>
 </template>

 <template v-else>
 <p class="text-xs text-warm-muted italic">No crop set — full image will export.</p>
 </template>
 </section>
 ...
 <div class="flex justify-between items-center text-xs mb-1">
 <span>View zoom</span>
 <span class="font-display text-coral font-bold">{{ Math.round(zoom * 100) }}%</span>
 </div>
 <input
 v-model.number="zoom"
 type="range" min="0.25" max="3" step="0.05"
 :disabled="!hasImage"
 class="w-full accent-coral cursor-pointer disabled:opacity-30 mb-1"
 />
 <button
 :disabled="!hasImage"
 class="text-[10px] text-warm-muted hover:text-ink disabled:opacity-30 transition-colors underline underline-offset-2"
 @click="zoom = 1"
 >reset to 100%</button>
 <p class="text-xs text-warm-muted italic mt-1">Display only — doesn't affect export.</p>
 </section>
 ...
 :class="textMode ? 'border-coral text-coral bg-coral/10' : 'border-warm-border text-warm-muted'"
 >
 ...
 <div class="flex justify-between items-center text-xs mb-1">
 <span>Font size</span>
 <span class="font-display text-coral font-bold">{{ labelFontSize }}px</span>
 </div>
 <input v-model.number="labelFontSize" type="range" min="8" max="48" class="w-full accent-coral cursor-pointer" />
 </div>
 <div>
 <p class="text-xs mb-1">Color</p>
 <input v-model="labelColor" type="color" class="w-full h-7 border border-warm-border cursor-pointer p-px" />
 </div>
 <div>
 ...
 :class="labelBold ? 'bg-ink dark:bg-[#1C1C1E] text-white border-ink' : 'border-warm-border hover:border-ink'"
 @click="labelBold = !labelBold"
 >B</button>
 <button
 class="px-2 py-1 text-xs border transition-colors italic"
 :class="labelItalic ? 'bg-ink dark:bg-[#1C1C1E] text-white border-ink' : 'border-warm-border hover:border-ink'"
 ...
 <select v-model="labelFont" class="w-full text-xs border border-warm-border bg-cream px-2 py-1 cursor-pointer appearance-none">
 ...
 <p class="text-xs text-warm-muted italic mt-2">
 ...
 >
 <!-- Empty state -->
 <div v-if="!hasImage" class="flex flex-col items-center justify-center h-full gap-2 text-warm-muted italic text-sm pointer-events-none">
 ...
 <div class="absolute border border-dashed border-coral/50 box-border"
 ...
 :key="label.id"
 class="absolute select-none border border-dashed border-transparent hover:border-coral px-1"
 :class="{ 'border-coral !border-solid': selectedLabelId === label.id }"
 :style="{
 left: label.x + 'px',
 top: label.y + 'px',
 ...
 class="absolute -top-2 -right-2 w-4 h-4 bg-coral text-white rounded-full text-[10px] flex items-center justify-center leading-none z-10"
